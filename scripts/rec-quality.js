...
const CHECKS = [
  ...
  ['personas: from their other five to seven favorites, at least 45% of hidden ones land in the top 100 (baseline 21%)',
    m => row(m, 'personas, pooled').engine.hitAt100 >= 0.45],
  ['personas: the average hidden favorite lands in the top 6.5% of ~5,000 untried titles (baseline 24%)',
    m => row(m, 'personas, pooled').engine.meanPercentile <= 6.5],
