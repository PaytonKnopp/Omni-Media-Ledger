...
- **The fast tier** (`npm run test-fast`, ~15s): corpus validation (4,995 works: 2,003 films,
  502 series, 506 games, 1,984 books; no duplicate ids or titles, every score in range), the
...
3. **Facts are mostly sourced; reception scores and the subjective indices are not, by design.** (Recounted 2026-09-25, after the duplicate merge; the "658 verified / 642 estimated" figure that used to be here dates from the 1,300-work corpus and the retired `PROV_CEIL` badge.) Three separate things, often conflated: **(a) the subjective indices** (dread, soundtrack, comfort, …) are judgement by definition — all 4,995 works are scored against RUBRIC.md (`indices: rubric-v1`) with recorded evidence, and no external source exists to verify them further. **(b) Facts** — year, runtime/pages, creator, studio/network/platforms — carry a per-record `prov.facts` stamp: movies 1,829 of 2,003 sourced or corroborated (125 edition-dependent, 49 estimated); TV 398 of 502 (104 estimated); books 548 of 1,984 (1,030 edition-dependent, which is the honest label for a page count, 406 estimated); **games 0 of 506 — all estimated**, because IGDB and Wikidata are unreachable from the cloud sessions this was built in, and they stay that way by owner decision (2026-09-25: not worth an IGDB developer account; QUALITY_PASS.md decision 14). These fields are mostly filter-only and carry almost no score leverage (see QUALITY_PASS.md's leverage ranking). **(c) Reception** (`criticalScore`/`audienceScore`) is not covered by the facts stamp; each reception value carries its own source stamp instead (RUBRIC.md "Reception fields"). **Film and TV `audienceScore` is sourced** (2026-09-25, Phase 49): IMDb's user rating ×10 for 2,502 of 2,505 works, each stamped `metrics.audienceSrc:{src:"IMDb",id,checked}`; the three IMDb has no title for (`m1879` Apocalypse Now Redux, `t315` Looney Tunes, `t316` Tom and Jerry) keep their estimate, stamped `{src:"estimated",why}`. **Still best estimates:** every `criticalScore` (no licensed critic source has been applied to any medium) and games' and books' `audienceScore` — the cards mark these "est." / "~". A 12-point error moves 🎯 GOAT Match by ~3.
...
**Real scheduled automation, via the platform rather than the static file.** Phase 8's assessment that quarterly refresh was "impossible in a static file with no server or cron" was correct about the file — it's still just JSON-shaped JS with no way to run code on a timer by itself. But this Claude Code Remote environment can: a cron-scheduled Routine now fires on each equinox/solstice turn (Sept 21, Dec 21, Mar 21, Jun 21, 15:00 UTC) into a **fresh** session that clones the repo, re-verifies all 50 entries the same way this phase did, curates out anything resolved or fabricated, runs `validate-corpus.js` and the full Playwright suite, and opens a PR (never merges it — that stays a human call). The runbook this produces isn't hypothetical anymore; it's the literal prompt behind the "Quarterly Contenders Ledger refresh" Routine (claude.ai/code → Routines). *(Update, 2026-09-22: the original trigger `trig_016uNv9UKRqzsf57G6qkACz8` no longer existed and the September 21 run never fired. It was recreated in the claude.ai Routines UI with this repo attached, cron `0 8 21 3,6,9,12 *` = 2am Calgary time on Mar/Jun/Sep 21 and 1am on Dec 21, when Mountain Standard Time is in effect. The run is now lighter: it edits only `data/contenders.js`, removes released, migrated and cancelled entries, backfills to exactly 50 with new ids (ids are never reused because the watchlist is keyed by id), then opens a PR and merges it itself, which the owner authorized. It runs on Sonnet, without subagents and without the Playwright suite, to keep usage low. The routine must have this repo attached as a source (set in the claude.ai Routines UI), or it cannot push.)* If this repo is ever cloned into an environment without that trigger, item 3 below still documents the manual runbook as a fallback.
...
## Phase 50 — Recommendations for every taste, a lint that fails, and the review backlog
...
reader's typical hidden favorite in the top five of ~5,000 and the comedy lover's past 400th. Two
measured causes. (1) The taste model learned only directions, and every comedy shares the Comedy
direction, so acclaim decided the order among them. `buildTasteModel` now also scores each work's
**closeness** to the specific works liked (`buildNeighborFit` in `app/scoring.js`), and the card
names the favorite it is closest to ("Like: …") when that favorite is the same medium and genuinely
alike. (2) The objective half assumed everyone's favorites are the acclaimed canon; measured, they
sit 0.12 SD above average for the comedy lover and 1.93 for the literary reader. `acclaimWeight()`
and `receptionMix()` now learn how much, and which, acclaim counts. Median rank of a hidden
favorite, the old engine against the new on the same IMDb-sourced data (the numbers first measured
before Phase 49 were within a few ranks of these): cosy-games 160 → 19 (top-100 2/6 → 5/6), family
drama 75 → 12, comedy 443 → 290, personas pooled 104 → 19 (top-100 50% → 64%), PK Sample 71 → 75
(top-100 28 → 27 of 53), literary fiction 4 → 14 (top-100 unchanged at 7/8). Floors raised so the
gain holds. A tier click's rescoring went from ~34 to ~56 ms. The comedy lover is still the weakest
profile, for a reason the data states plainly: eight comedies with little in common besides being
comedies, among ~600 in the corpus.
...
**Honest labels.** The Critical Score tooltip no longer claims a Tomatometer source, the Audience
Score tooltip says which audiences are IMDb's (Phase 49) and which are still estimates, and the
Warmth/Comic/Beauty tooltips stop saying those fields are unscored (all works are). The per-card
source labels are Phase 49's; an earlier blanket "critic & audience scores estimated" note from this
branch was dropped in the merge, since film and TV audiences are no longer estimates.
...
eight): after onboarding's start button the app saves and reloads, and the reloaded page
occasionally came up with an empty localStorage (every key present in the old page right before
`location.reload()`, none in the new one as its boot began, sessionStorage intact), so it rightly
showed the sign-in gate. Nothing in the app removes them in between, and it never happened in 760
write-and-reload cycles outside the suite. The owner check now starts signed in (seeded
...
6. **Raise data provenance.** Mostly done for facts; see Known limitations #3 above for the current counts (recounted 2026-09-25). **Film/TV audience scores: done** (Phase 49 — IMDb for 2,502 of 2,505, the other three labelled estimates). What remains, in order of value: the other **reception scores** — games' audience and critic scores (IGDB's aggregated rating), books' (no aggregator exists for most; decide whether they stay labelled estimates), and film/TV critic scores (RT and Metacritic have no licensed bulk source, so these may stay estimates for good); one source per field, applied uniformly, never averaged across catalogues — then **game facts** (506 estimated, and staying so by owner decision, QUALITY_PASS.md #14; `scripts/fetch-facts.js` already has an IGDB adapter). IGDB needs a Twitch client id/secret, which the cloud sessions do not have — run `fetch-facts` locally with free keys per DATA_RUNBOOK.md, or add them to the environment. Refresh the IMDb values by re-running Phase R of DATA_RUNBOOK.md with a newer `title.ratings.tsv.gz`. Search-engine summaries are evidence grade B and are deliberately not used to overwrite estimates. Low priority: the leverage is small next to the taste signals, and nothing here is wrong so much as unconfirmed.
...
**Batches 36-37:** movies 96 (biopics/prestige dramas + social-issue films), books 87 (YA fantasy series completions + MG/classic-catalog sweep). Scaled batch size up sharply per user request to finish faster. Caught two new issues: 74 new movie records missing `physicalMediaFidelity` entirely (insert-batch.js candidates lacked transfer/audio/cine fields — patched via a beauty-score-derived formula script) and a true content duplicate ("Prince Caspian" vs new "The Chronicles of Narnia: Prince Caspian" — removed the new one) plus a creator-spelling mismatch ("C.S. Lewis" vs corpus's "C. S. Lewis", fixed to match). Checkpoint: movies 1,925/2,000 (75 to go), books 1,758/2,000 (242 to go).

**Batches 38-39:** movies 81 (heist/legal-thriller sweep + classic Hollywood musical catalog), books 243 (Christie/Sayers/Collins mystery catalog, Wodehouse Jeeves & Blandings, Golden Age SF completions, Ballard/Strugatsky/Lem New Wave SF, and a large postmodern literary sweep across Cheever/Updike/Malamud/Bellow/Roth/Nabokov/Barth/Pynchon/DeLillo/DFW/Franzen, plus 3 Gaskell novels to close out). Found and fixed a pre-existing near-duplicate from batch 35 (two punctuation variants of Jekyll and Hyde) and a genre-taxonomy misuse ("Non-Fiction" used as a genre tag instead of formatType). Repeated the missing-`physicalMediaFidelity` fix from batch 36-37 for this batch's movies too.

**Expansion complete.** Final counts: movies 2,006/2,000, TV 502/500, games 507/500, books 2,000/2,000 — total 5,015 works, every one scored under rubric-v1 with sourced or clearly-disclosed general-knowledge-estimate evidence.
