...
function renderContenders(){const MED={Film:'#a78bfa',TV:'#22d3ee',Game:'#fbbf24',Book:'#4ade80'};
 ...
 // Anything no longer upcoming -- released, shelved, or past an exact date with no status update
 // yet -- sinks below everything that still is, whatever the sort. Dates pass between the quarterly
 // refreshes; this keeps a stale entry from leading a list of what is coming next.
 var settled=function(c){return c.migratedTo||/^Released/.test(c.window||'')||isShelved(c)||isPastWindow(c)?1:0;};
 var bySort=(
 ...
 pool.sort(function(a,b){return (settled(a)-settled(b))||bySort(a,b);});
...
/* What the personal half of every match is built on, in words -- "12 ratings, 5 favorites and 40
   ...
   tier, owned title or hand-set boost. Every score is then just critical, audience and
...
function tasteBasis(){
 ...
 _basis={personal:parts.length>0,rated:rated,fav:fav,owned:owned,boosts:boosts,text:text};
...
function mutateProfile(mutatorFn,forceFullRender){
 ...
 // "Not interested" was removed; a list left over from it has nothing left to read it.
 delete snapshot.notInterested;
