...
function creatorCard(c,tab,ALL){const isDir=tab===true||tab==='directors';const isAuthor=tab==='authors';const isAuteur=tab==='auteurs'||tab===false;const works=worksFor(c.name,ALL);const accent=isDir?'#a78bfa':(isAuthor?'#4ade80':'#fbbf24');
 ...
 const avgCrit=works.length?(works.reduce(function(s,w){return s+w.crit;},0)/works.length):0;
 const countNode=works.length
  ?'<div class="text-2xl font-extrabold leading-none goatJump cursor-pointer" data-q="'+esc(c.name)+'" title="View all of '+esc(c.name)+'’s works in the Global Controller" style="color:'+accent+'">'+works.length+'</div>'
  :'<div class="text-2xl font-extrabold leading-none" style="color:'+accent+'">'+works.length+'</div>';
 const front='<div class="flip-face absolute inset-0 panel p-4 flex flex-col">'
  ...
  +'<div class="text-right shrink-0">'+countNode+'<div class="lbl mt-1">on ledger</div></div></div>'
  +'<div class="flex flex-wrap gap-1.5 mt-3">'+c.activeEras.map(e=>'<span class="chip">'+esc(e)+'</span>').join('')+'</div>'
  +(works.length?'<div class="flex items-center gap-1.5 mt-2 text-[10.5px]" title="'+ownedN+' of '+works.length+' ledger works owned"><div class="flex-1 h-1 rounded-full bg-slate-800 overflow-hidden"><div style="width:'+ownedPct+'%;height:100%;background:'+accent+'"></div></div><span class="text-slate-400 tabular-nums shrink-0">'+ownedPct+'% owned</span><span class="text-slate-500 tabular-nums shrink-0">·&nbsp;'+avgCrit.toFixed(1)+' avg</span></div>':'')
...
function avgCritFor(name,ALL){const works=worksFor(name,ALL);return works.length?(works.reduce(function(s,w){return s+w.crit;},0)/works.length):-1;}
function sortCreatorPairs(pairs,sortMode,ALL){
 ...
 if(sortMode==='rating')return pairs.slice().sort(function(a,b){return avgCritFor(b[0].name,ALL)-avgCritFor(a[0].name,ALL);});
 return pairs;
}
function filterCreatorPairs(pairs,state,ALL){
 var out=pairs;
 if(state.creatorLedgerOnly)out=out.filter(function(pair){return worksFor(pair[0].name,ALL).length>0;});
 if(state.creatorOwnedOnly)out=out.filter(function(pair){return worksFor(pair[0].name,ALL).some(function(w){return w.owned;});});
 return out;
}
